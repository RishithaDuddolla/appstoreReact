...
class AppStore extends Component {
  state = {searchedFor: '', activeTab: tabsList[0].tabId}
  ...
  render() {
    const {searchedFor, activeTab} = this.state
    const apps = appsList.filter(eachapp => eachapp.category === activeTab)
    const searchedapps = apps.filter(eachapp =>
      eachapp.appName
        .toLocaleLowerCase()
        .includes(searchedFor.toLocaleLowerCase()),
    )
    return (
